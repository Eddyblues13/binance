...
<head>
    ...
    <title>Low and Attractive Day Trading Margins | Plus500</title>
        <meta name="description" content="Trade Futures with low intraday margins for day trading" />


        <link rel="canonical" href="https://us.plus500.com/margins" />
    ...
    <link rel="stylesheet" href="https://cdn-main.plus500.com/1.0.0.120805/Resources/CSS/futures/margins.css"/>
    ...
    <script>
        ...
        <script>
            ...
            var analyticsParameters = {};
                        ...
                        analyticsParameters['pageType']='FuturesMargins'
    ...
    <link rel="alternate" href="https://us.plus500.com/margins" hreflang="en-US" />
    
</head>
<body class="futuresmargins regulator-cftc country-us lang-en Win10">
                                        ...
                                        <table class="table-child" role="presentation">
                                            ...
                                            <tbody>
                                                ...
                                                <tr class="row-data">
                                                    ...
                                                    <td class="symbol">HEZ4</td>
                                                    <td class="intra-day">$250</td>
                                                </tr>
                                                <tr class="row-data">
                                                    <td class="name">Live Cattle</td>
                                                    <td class="symbol">LEZ4</td>
...
<a data-analytics="buttonClick" class="side-support-link support-link fade" href="/support" title="24/7 Support">
    ...
    <script>
        window.addEventListener("DOMContentLoaded", () => {
            const tablesSwitcher = document.querySelector("#tableSwitcher");
            const tablesWrapper = document.querySelector("#tablesWrapper");
            const microMiniTable = document.querySelector(".micro-mini-table");
            const contractsTable = document.querySelector(".contracts-table");
            const THRESHOLD = 40; // Y axis threshold, in pixels
            ...
            const calcTablesWrapperHeight = (el) => { 
                tablesWrapper.style.maxHeight = (!el.checked ? microMiniTable.offsetHeight : contractsTable.offsetHeight) + "px"
            }
            const handleTouchstart = (event) => {
                touchstartX = !!event && !!event.changedTouches && event.changedTouches.length && !!event.changedTouches[0] ? event.changedTouches[0].clientX : 0;
                touchstartY = !!event && !!event.changedTouches && event.changedTouches.length && !!event.changedTouches[0] ? event.changedTouches[0].clientY : 0;
            }
            const handleGesture = (event) => {
                let touchendX = !!event && !!event.changedTouches && event.changedTouches.length && !!event.changedTouches[0] ? event.changedTouches[0].clientX : 0,
                    touchendY = !!event && !!event.changedTouches && event.changedTouches.length && !!event.changedTouches[0] ? event.changedTouches[0].clientY: 0;
                
                // if Y axis threshold exceeded then return, a user was intentionally scrolling
                if (touchstartY > touchendY && touchstartY-touchendY > THRESHOLD || touchstartY < touchendY && touchendY-touchstartY > THRESHOLD){
                    return;
                }
                
                if (touchendX < touchstartX){ 
                    tablesSwitcher.checked = true;    
                } // right
                if (touchendX > touchstartX) {
                    tablesSwitcher.checked = false;
                 } // left    
            }
            const tableResizeObserver = new ResizeObserver((entries) => {
              for (const entry of entries) {
                calcTablesWrapperHeight(tablesSwitcher);
              }
            });
            calcTablesWrapperHeight(tablesSwitcher);
            tablesSwitcher.addEventListener("change", (e) => {
                calcTablesWrapperHeight(e.target);
            });
            tablesWrapper.addEventListener("touchstart", (e) => {
                handleTouchstart(e);
            });
            tablesWrapper.addEventListener("touchend", (e) => {
                handleGesture(e);
                calcTablesWrapperHeight(tablesSwitcher);
            });
            ...
            for (const child of tablesWrapper.children){
                tableResizeObserver.observe(child);
            }
        });
    </script>
